...
def generate_parent(map_x,map_y):
    used_positions = []
    parent = []

    for x in range(const.MONSTER_COUNT):
        monster_position = generate_position(used_positions,map_x,map_y)
        monster_type = const.MONSTER_NAMES[randint(0, len(const.MONSTER_NAMES)-1)]
...
def fitness_function(parent,difficulty):
    ...
    for monster_name in const.MONSTER_NAMES:
    ...
    for x in parent:
        ...
        distances.append(calculate_distance(const.START_POS,x[0]))  # distances between witcher and certain monster |  ex. [11, 23, 19, 11, 20, 19, 15, 15, 11, 13]
...
def crossbreed(spawnlist1,spawnlist2,map_x,map_y,difficulty):
    used_positions1 = []
    used_positions2 = []
    ...
    for iter in range(len(spawnlist1)+1,0,-1):  # get rid of redundant elements
        ...
        while used_positions2[i] in used_positions2[:i]:
            new_pos = generate_position(used_positions2,map_x,map_y)
            spawnlist2[i][0] = new_pos
            used_positions2[i] = new_pos
    ...
    rerolled_monster = const.MONSTER_NAMES[randint(0, len(const.MONSTER_NAMES)-1)]
    ...
    #print(f'list1: {spawnlist1}\nlist2: {spawnlist2}\nused_positions1: {used_positions1}\nused_positions2: {used_positions2}\nfit1={fitness_function(spawnlist1,difficulty)}, fit2={fitness_function(spawnlist2,difficulty)}\n\n')

    return [spawnlist1,fitness_function(spawnlist1,difficulty)],[spawnlist2,fitness_function(spawnlist2,difficulty)] # return mixed pair
...
def algorithm(difficulty,map_x,map_y):
    ...
    for parent_id in range(population) :
        parent = generate_parent(map_x,map_y)
        fitness = fitness_function(parent,difficulty)
    ...
    for iter in range(mutations):
        ...
        for pairs in range(int(len(breed)/2)):
            ...
            #print(f'{2*pairs+1}:\n{breed[2*pairs+1]}\n')
            #print('----------------')
            breed[2*pairs],breed[2*pairs+1] = crossbreed(breed[2*pairs][0],breed[2*pairs+1][0],map_x,map_y,difficulty) # mix 2 parents
            #print('----------------')
            #print(f'{2*pairs}:\n{breed[2*pairs]}')
            #print(f'{2*pairs+1}:\n{breed[2*pairs+1]}\n')
        universe[-1*to_breed:] = breed     # replaced worst elements by new ones
        #print(universe[-1])
...
result = algorithm(0,0,0) # algorithm(difficulty, map_x, map_y) | LAUNCH EXAMPLE
